...
// =========================== MAHONY FILTER FUNCTIONS =======================
uint8_t Mahony_init(MahonyFilter_t *mahony){
	mahony->kp = 0.5f; // Valor inicial recomendado para rápida convergencia
	mahony->ki = 0.01f; // Inicialmente sin integral
	mahony->dt = 0.0f;
	mahony->time_stamp = HAL_GetTick();

	mahony->integral_error_vector[0] = 0.0f;
	mahony->integral_error_vector[1] = 0.0f;
	mahony->integral_error_vector[2] = 0.0f;

	// Estado plano / identidad (Normalizado a 1.0)
	mahony->unit_quaternion[0] = 1.0f;
	mahony->unit_quaternion[1] = 0.0f;
	mahony->unit_quaternion[2] = 0.0f;
	mahony->unit_quaternion[3] = 0.0f;

	mahony->euler_angles.roll  = 0.0f;
	mahony->euler_angles.pitch = 0.0f;
	mahony->euler_angles.yaw   = 0.0f;
...
void Mahony_compute(MahonyFilter_t * mahony, MPU6050_t * sensor){
	//calculate dt
	uint32_t this_moment = HAL_GetTick();
	mahony->dt = (float)(this_moment - mahony->time_stamp)/1000.0f;
	mahony->time_stamp = this_moment;

	float ax = sensor->scaled_data.ax;
	float ay = sensor->scaled_data.ay;
	float az = sensor->scaled_data.az;

	 // Convertimos giroscopio de deg/s a rad/s si es necesario
	float gx = sensor->scaled_data.gx * DEG_TO_RAD_MAHONY;
	float gy = sensor->scaled_data.gy * DEG_TO_RAD_MAHONY;
	float gz = sensor->scaled_data.gz * DEG_TO_RAD_MAHONY;
// STEP 1: Normmalize acceleration vector
	float accel_norm_sq = ax*ax + ay*ay + az*az;
	// Check if the square norm  is above 0. This prevents an error if the MPU6050 is not responding.
	if(accel_norm_sq > 0.0001f) {
		float inv_accel_norm = 1.0f / sqrtf(accel_norm_sq);
	     ax *= inv_accel_norm;
	     ay *= inv_accel_norm;
	     az *= inv_accel_norm;

	// STEP 2: Stiamte gravity
	     float q0 = mahony->unit_quaternion[0];
		 float q1 = mahony->unit_quaternion[1];
		 float q2 = mahony->unit_quaternion[2];
		 float q3 = mahony->unit_quaternion[3];

		 float vx = 2.0f * (q1*q3 - q0*q2);
		 float vy = 2.0f * (q0*q1 + q2*q3);
		 float vz = q0*q0 - q1*q1 - q2*q2 + q3*q3;

	// STEP 3: Calculate quaternion's angular velocity error
		 float ex = (ay * vz - az * vy);
		 float ey = (az * vx - ax * vz); // Corregido orden
		 float ez = (ax * vy - ay * vx); // Corregido orden

	// Save the integral error of the bias
		 mahony->integral_error_vector[0] += ex * mahony->dt;
		 mahony->integral_error_vector[1] += ey * mahony->dt;
		 mahony->integral_error_vector[2] += ez * mahony->dt;

	// STEP 4: Calculate angular velocity correction vector
		 gx += mahony->kp * ex + mahony->ki * mahony->integral_error_vector[0];
		 gy += mahony->kp * ey + mahony->ki * mahony->integral_error_vector[1];
		 gz += mahony->kp * ez + mahony->ki * mahony->integral_error_vector[2];
	}
	// STEP 5; calculate quaternion's angular velocity

	float q0_old = mahony->unit_quaternion[0];
	float q1_old = mahony->unit_quaternion[1];
	float q2_old = mahony->unit_quaternion[2];
	float q3_old = mahony->unit_quaternion[3];

	float q0dot = 0.5f * (-q1_old*gx - q2_old*gy - q3_old*gz);
	float q1dot = 0.5f * ( q0_old*gx + q2_old*gz - q3_old*gy);
	float q2dot = 0.5f * ( q0_old*gy - q1_old*gz + q3_old*gx);
	float q3dot = 0.5f * ( q0_old*gz + q1_old*gy - q2_old*gx);
	// STEP 6: calculate the new quaternion position and normalize
	float q0_new = q0_old + q0dot * mahony->dt;
	float q1_new = q1_old + q1dot * mahony->dt;
	float q2_new = q2_old + q2dot * mahony->dt;
	float q3_new = q3_old + q3dot * mahony->dt;

	float inv_q_norm = 1.0f / sqrtf(q0_new*q0_new + q1_new*q1_new + q2_new*q2_new + q3_new*q3_new);

	// we calculate de inverse of the norme and multiply because multiplying is faster action than dividing
	mahony->unit_quaternion[0] = q0_new * inv_q_norm;
	mahony->unit_quaternion[1] = q1_new * inv_q_norm;
	mahony->unit_quaternion[2] = q2_new * inv_q_norm;
	mahony->unit_quaternion[3] = q3_new * inv_q_norm;
...
void Mahony_ToEuler(MahonyFilter_t * mahony){
	...
	// Test for the pitch singularity (Gimbal Lock Check)
	float test = q0*q2 - q1*q3;

	if (test > 0.499f) { // Process of the north singularity  (Pitch = +90 deg)
	   mahony->euler_angles.pitch = 90.0f;
	   mahony->euler_angles.yaw = 2.0f * atan2f(q1, q0) * RAD_TO_DEG_MAHONY;
	   mahony->euler_angles.roll = 0.0f; // Asignamos todo el giro al Yaw para evitar indefinición
	}
	else if (test < -0.499f) { // Process of the south singularity (Pitch = -90 deg)
	   mahony->euler_angles.pitch = -90.0f;
	   mahony->euler_angles.yaw = -2.0f * atan2f(q1, q0) * RAD_TO_DEG_MAHONY;
	   mahony->euler_angles.roll = 0.0f;
	}
	else {
	   // Conversión normal sin singularidad
	   mahony->euler_angles.roll = atan2f(2.0f*(q0*q1 + q2*q3), 1.0f - 2.0f*(q1*q1 + q2*q2)) * RAD_TO_DEG_MAHONY;
	   mahony->euler_angles.pitch = asinf(2.0f*test) * RAD_TO_DEG_MAHONY;
	   mahony->euler_angles.yaw = atan2f(2.0f*(q0*q3 + q1*q2), 1.0f - 2.0f*(q2*q2 + q3*q3)) * RAD_TO_DEG_MAHONY;
	}
}
